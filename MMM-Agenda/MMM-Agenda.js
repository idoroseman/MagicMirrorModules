...
Module.register("MMM-Agenda", {
	...
	getStyles: function () {
		return ["MMM-Agenda.css", "font-awesome.css"];
	},
	...
	getDom: function () {
      ...
      var eventDate = moment(event.startDate, "x");
			if (eventDate.format("YYYY-MM-DD") != currDate){
				...
				var titleDate = moment(currDate,"YYYY-MM-DD")
				var headerWrapper = document.createElement("tr");
				headerWrapper.className = "daychange";
				var dateWrapper = document.createElement("td");
				dateWrapper.colSpan = "3";
			...
			if (event.fullDayEvent) {
					timeWrapper.innerHTML = this.capFirst(this.translate("ALLDAY"));
			} else {
					timeWrapper.innerHTML = this.capFirst(moment(event.startDate, "x").format("LT"));
			}
	...
	getLocaleSpecification: function(timeFormat) {
		...
		case 24: {
			return { longDateFormat: {LT: "HH:mm"}, calendar: {nextWeek:'dddd [ב]LT' }};
			break;
		}
		default: {
